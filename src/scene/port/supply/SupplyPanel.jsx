    ...
    const render = useCallback(() => {
        if (currentFleet === -1) {
            ...
            fleet.forEach(ship_no => {
                ...
                if (!(target_ship.api_onslot.length === target_ship_base_info.api_maxeq.length && target_ship.api_onslot.every((value, index) => value === target_ship_base_info.api_maxeq[index]))) {
                    flightNeedSupply = true;
                }
            })
            ...
            return <>
                ...
                <Text text={fuelToSupply} x={1045} y={300} anchor={{ x: 1, y: 0 }} style={{ fill: 'black', fontSize: 32 }} />
                {/* 補給の弾薬 */}
                ...
                <Text text={bullToSupply} x={1185} y={300} anchor={{ x: 1, y: 0 }} style={{ fill: 'black', fontSize: 32 }} />
                {/* 艦載機補充 */}
                <PixiButton
                    default={supplyMain[15]}
                    disabled={supplyMain[16]}
                    isDisabled={!flightNeedSupply}
                    hover={supplyMain[17]}
                    action={resetShipSelection}
                    x={915} y={597}
                />
                {/* 艦隊全補給 */}
                <PixiButton
                    default={supplyMain[12]}
                    disabled={supplyMain[13]}
                    isDisabled={!fleetNeedSupply}
                    hover={supplyMain[14]}
                    actionOver={SelectAllShip}
                    actionOut={resetShipSelection}
                ...
                {/* 燃料補給 */}
                <PixiButton
                    default={supplyMain[9]}
                    disabled={supplyMain[10]}
                    isDisabled={!shipSelection.includes(true)}
                    hover={supplyMain[11]}
                    action={resetShipSelection}
                    x={918} y={641} />
                {/* まとめて補給 */}
                <PixiButton
                    default={supplyMain[3]}
                    disabled={supplyMain[4]}
                    isDisabled={!shipSelection.includes(true)}
                    hover={supplyMain[5]}
                    action={resetShipSelection}
                    x={979} y={641} />
                {/* 弾薬補給 */}
                <PixiButton
                    default={supplyMain[6]}
                    disabled={supplyMain[7]}
                    isDisabled={!shipSelection.includes(true)}
                    hover={supplyMain[8]}
                    action={resetShipSelection}
                    x={1135} y={641} />
