...
class GameUtils:
    ...
    def update_ship_status_with_slot_items(ship):
        ...
        # 索敌
        new_sakuteki = GameUtils.get_ship_base_sakuteki(ship.api_ship_id, ship.api_lv)
        # 回避
        new_kaihi = GameUtils.get_ship_base_kaihi(ship.api_ship_id, ship.api_lv)
        # 对潜
        new_taisen = GameUtils.get_ship_base_taisen(ship.api_ship_id, ship.api_lv)
        # 计算每个装备的加成
                    ...
                    if ship_mst_slotitem_id in bonus_setting["item_id"]:
                        ...
                        for target_bonus_setting in bonus_setting["target"]:
                            if ship.api_ship_id in target_bonus_setting["ship_id"]:
                                ...
                                bonus = GameUtils.get_bonus_setting_by_slotitem_level(
    ...
    def calculate_repair_complete_time(ship):
        ...
        return int(time.time() * 1000 + repair_seconds * 1000)
