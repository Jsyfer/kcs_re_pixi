...
def charge(request):
    ...
    for ship_id in ship_id_list:
        ...
        if api_kind == "0":
            # 舰载机补充
            aluminium_consumption += (sum(mst_ship.api_maxeq or []) - sum(ship.api_onslot or [])) * 5
            # 更新舰船状态
    ...
    # 更新各资源消耗
    if api_kind == "3":
        material.fuel -= fuel_consumption
        material.bull -= bull_consumption
    material.aluminium -= aluminium_consumption
    material.save()
